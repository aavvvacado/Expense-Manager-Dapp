...
contract ExpenseManagerContract {
    ...
    function deposit(uint _amount, string memory _reason) public payable {
        ...
        transactions.push(
            Transaction(msg.sender, _amount, _reason, block.timestamp, TransactionType.DEPOSIT)
        );
    ...
    function withdraw(uint _amount, string memory _reason) public {
        ...
        balances[msg.sender] -= _amount;
        transactions.push(
            Transaction(msg.sender, _amount, _reason, block.timestamp, TransactionType.WITHDRAWAL)
        );
    ...
    function getTransaction(
        uint _index
    ) public view returns (address, uint, string memory, uint, TransactionType) {
        require(
        ...
        )
    {
        ...
        TransactionType[] memory transactionTypes = new TransactionType[](transactions.length);
